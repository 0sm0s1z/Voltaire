...
print "<style>.topic{color: blue;font-weight: bold;}.title{font-size: 60; font-weight: bold;}</style>"
...
for raw in doc.readlines():
    for datz in raw.split("\r"):
        ...
        try:
           title = data.split("\t")[0]
           title2 = re.sub('[#$"]', '', title[:1])
           title = title2.capitalize() + title[1:]
           topic.append(cgi.escape(data.split("\t")[0]))
        except:
		   pass
        try:
            #title = data.split("\t")[1]
            ...
            #title = title2.capitalize() + title[1:]
            #print title
            definition.append(re.sub('["]', '', data.split("\t")[1]))
...
for blah in topic:
    try:
		index.append([topic[i], course[i], book[i], page[i], definition[i]])
...
for item in index:
    ...
    #Print Details
    if item[0] != "":
        print "<span class = 'topic'>%s</span> <i>[%sb%s/p%s]</i>  %s<br>" % (cgi.escape(item[0].strip('"').rstrip('"')), item[1].rstrip(" "), item[3], item[2].strip('"').rstrip('"'), cgi.escape(item[4].rstrip('"')))
		#print item[0]
		#pass
